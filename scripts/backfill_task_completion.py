...
class TaskCompletionBackfill:
    ...
    def run(self) -> dict:
        ...
        logger.info(f"Backfill complete: {stats['tasks_completed']}/{stats['tasks_checked']} tasks marked complete")
    ...
    def _is_task_completed(self, task: dict) -> bool:
        ...
        title_words = {word for word in re.findall(r"\w+", task_title) if len(word) > 3 and word not in self.stop_words}
