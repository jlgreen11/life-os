...
def test_reciprocity_high_outbound_uses_display_name(db):
    ...
    with db.get_connection("entities") as conn:
        ...
        conn.execute(
            "INSERT INTO contact_identifiers (contact_id, identifier, identifier_type) VALUES (?, ?, ?)",
...
async def test_relationship_intelligence_wired_into_generate_insights(db):
    ...
    rel_insights = [
        i for i in all_insights if i.type == "relationship_intelligence" and i.category == "reciprocity_imbalance"
    ]
