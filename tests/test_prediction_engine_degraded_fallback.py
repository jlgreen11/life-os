...
def _populate_bidirectional_contact(
    event_store, contact_email, inbound_count=6, outbound_count=2, last_interaction_days_ago=20
):
...
@pytest.mark.asyncio
async def test_relationship_maintenance_falls_back_to_events_db_on_exception(db, event_store, user_model_store):
    ...
    assert len(predictions) >= 1, f"Expected >=1 prediction from events.db fallback, got {len(predictions)}"
...
@pytest.mark.asyncio
async def test_relationship_maintenance_fallback_has_conservative_confidence(db, event_store, user_model_store):
    ...
    for pred in predictions:
        assert 0.3 <= pred.confidence <= 0.6, f"Fallback confidence {pred.confidence} outside expected range [0.3, 0.6]"


@pytest.mark.asyncio
async def test_relationship_maintenance_uses_signal_profile_when_healthy(db, event_store, user_model_store):
    ...
    profile_data = {
        "contacts": {
            "carol@example.com": {
                ...
                "interaction_timestamps": [(now - timedelta(days=30 + i * 5)).isoformat() for i in range(10)],
            }
        }
    }
    ...
    with patch.object(engine, "_build_contacts_from_events", wraps=engine._build_contacts_from_events) as mock_build:
        predictions = await engine._check_relationship_maintenance({})
...
@pytest.mark.asyncio
async def test_relationship_maintenance_no_predictions_without_data(db, event_store, user_model_store):
...
@pytest.mark.asyncio
async def test_routine_deviations_returns_empty_when_degraded(db, event_store, user_model_store):
...
@pytest.mark.asyncio
async def test_routine_deviations_runs_normally_when_healthy(db, event_store, user_model_store):
...
@pytest.mark.asyncio
async def test_follow_up_needs_works_with_degraded_user_model(db, event_store, user_model_store):
...
@pytest.mark.asyncio
async def test_follow_up_needs_confidence_without_priority_boost(db, event_store, user_model_store):
...
@pytest.mark.asyncio
async def test_generate_predictions_produces_output_with_degraded_user_model(db, event_store, user_model_store):
