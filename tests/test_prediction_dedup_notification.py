    ...
    if first_run:
        ...
        with db.get_connection("user_model") as conn:
            for pred in second_run:
                row = conn.execute("SELECT id FROM predictions WHERE id = ?", (pred.id,)).fetchone()
                assert row is not None, (
                    f"Returned prediction {pred.id} does not exist in DB — notification would be orphaned"
                )
