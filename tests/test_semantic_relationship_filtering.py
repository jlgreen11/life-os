...
def test_all_oneway_contacts_skips_inference(user_model_store):
    ...
    assert len(relationship_facts) == 0, "No relationship facts should be generated when all contacts are one-way"
...
def test_logging_when_no_bidirectional_contacts(user_model_store, caplog):
    ...
    assert "No human bidirectional contacts found" in caplog.text or "No bidirectional contacts found" in caplog.text, (
