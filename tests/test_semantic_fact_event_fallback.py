...
def _insert_email_received(db, from_address: str, subject: str = "Hello", hour: int = 10, day_offset: int = 0) -> None:
    ...
    with db.get_connection("events") as conn:
        conn.execute(
            """INSERT INTO events
               ...
               VALUES (?, ?, ?, ?, ?, ?, ?)""",
            (str(uuid.uuid4()), "email.received", "proton_mail", ts, "normal", payload, "{}"),
        )


def _insert_bulk_email_received(db, contacts: list[tuple[str, int]], hour: int = 10) -> None:
...
class TestEventFallbackRelationshipFacts:
    ...
    def test_top_contacts_produce_relationship_facts(self, db, user_model_store):
        ...
        assert result["facts_written"] >= 5, "Expected at least 1 fact per contact (5 contacts x 4 emails each)"
        ...
        assert len(contact_facts) >= 5, (
            f"Expected 5 contact facts, got {len(contact_facts)}: {[f['key'] for f in contact_facts]}"
        )
    ...
    def test_work_vs_personal_classification(self, db, user_model_store):
        ...
        fallback_facts = _get_all_fallback_facts(user_model_store)
        contact_facts = {
            f["value"]["email"]: f["value"]
            for f in fallback_facts
            if "contact" in f["key"] and "email" in f.get("value", {})
        }

        assert "colleague@bigcorp.com" in contact_facts, "Work contact missing from facts"
        assert contact_facts["colleague@bigcorp.com"]["relationship_type"] == "work"

        assert "friend@gmail.com" in contact_facts, "Personal contact missing from facts"
    ...
    def test_marketing_senders_excluded(self, db, user_model_store):
        ...
        fallback_facts = _get_all_fallback_facts(user_model_store)
        contact_facts = {
            f["value"]["email"]: f for f in fallback_facts if "contact" in f["key"] and "email" in f.get("value", {})
        }

        assert "no-reply@amazon.com" not in contact_facts, "Marketing sender should be filtered out"
        assert "noreply@paypal.com" not in contact_facts, "No-reply sender should be filtered out"
        assert "real_person@company.com" in contact_facts, "Real human contact should be present"
    ...
    def test_contact_below_minimum_count_excluded(self, db, user_model_store):
        ...
        assert "rare@company.com" not in contact_emails, "Contact with only 1 email should not produce a fact"
    ...
    def test_max_ten_contact_facts_written(self, db, user_model_store):
        ...
        fallback_facts = _get_all_fallback_facts(user_model_store)
        contact_facts = [f for f in fallback_facts if "contact" in f["key"]]
        assert len(contact_facts) <= 10, f"Expected at most 10 contact facts, got {len(contact_facts)}"
...
class TestEventFallbackTemporalFacts:
    ...
    def test_active_hours_fact_created(self, db, user_model_store):
        ...
        assert 9 in active_hours_fact["value"], f"Expected hour 9 in active_hours, got {active_hours_fact['value']}"
    ...
    def test_most_active_day_fact_created(self, db, user_model_store):
        ...
        assert active_day_fact is not None, "Expected event_fallback_most_active_day fact"
...
class TestEventFallbackTopicFacts:
    ...
    def test_frequent_subject_words_produce_topic_facts(self, db, user_model_store):
        ...
        for i, subject in enumerate(subjects):
            _insert_email_received(
                db,
                f"colleague{i}@work.com",
                subject=subject,
                hour=10,
                day_offset=i,
            )
        # Insert a 5th to ensure 'budget' appears ≥ 2 times with different senders
        _insert_email_received(db, "finance@work.com", subject="Budget review", hour=10, day_offset=4)
        ...
        assert len(topic_facts) >= 1, "Expected at least one topic fact from subjects containing 'budget'"
        topic_keys = [f["key"] for f in topic_facts]
        assert "event_fallback_topic_budget" in topic_keys, f"Expected 'budget' topic fact, got: {topic_keys}"
    ...
    def test_re_fwd_prefixes_stripped(self, db, user_model_store):
        ...
        fallback_facts = _get_all_fallback_facts(user_model_store)
        topic_keys = [f["key"] for f in fallback_facts if f["key"].startswith("event_fallback_topic_")]
        ...
        assert "event_fallback_topic_project" in topic_keys, f"Expected 'project' topic fact, got: {topic_keys}"
    ...
    def test_short_words_excluded(self, db, user_model_store):
        ...
        assert "event_fallback_topic_analysis" in topic_keys or "event_fallback_topic_data" in topic_keys, (
...
class TestEventFallbackGate:
    ...
    def test_fallback_triggered_when_no_episodes(self, db, user_model_store):
        ...
        fallback_result = next(
            (r for r in inferrer._last_inference_results if r.get("type") == "event_fallback"),
            None,
        )
        assert fallback_result is not None, "Expected 'event_fallback' entry in _last_inference_results"
    ...
    def test_fallback_not_triggered_when_episodes_exist(self, db, user_model_store):
        ...
        assert fallback_result is None, (
            f"Fallback should be skipped when episodes exist; found result: {fallback_result}"
        )
    ...
    def test_fallback_not_triggered_below_event_threshold(self, db, user_model_store):
        ...
        assert fallback_result is None, f"Fallback should be skipped with <= 100 events; found: {fallback_result}"
...
class TestEventFallbackEdgeCases:
    ...
    def test_missing_payload_fields_skipped_gracefully(self, db, user_model_store):
        ...
        fallback_facts = _get_all_fallback_facts(user_model_store)
        contact_facts = [f for f in fallback_facts if "contact" in f["key"]]
        assert len(contact_facts) == 0, "Expected no contact facts from an event with no from_address"
