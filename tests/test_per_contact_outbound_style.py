...
class TestPerContactAveragesComputed:
    ...
    def test_per_contact_averages_computed_after_min_samples(self, user_model_store):
        ...
        assert "alice@example.com" in pca, f"alice should appear in per_contact_averages after {min_samples} messages"
    ...
    def test_per_contact_averages_absent_below_min_samples(self, user_model_store):
        ...
        assert "bob@example.com" not in pca, (
            f"bob should NOT appear in per_contact_averages — only {min_samples - 1} messages (below threshold)"
        )
    ...
    def test_per_contact_averages_correct_values(self, user_model_store):
        ...
        assert abs(avg_formality - expected) < 0.01, f"formality avg {avg_formality:.3f} should equal {expected:.3f}"
    ...
    def test_multiple_contacts_tracked_independently(self, user_model_store):
        ...
        for _ in range(min_samples):
            extractor._update_profile(_make_signal("formal@example.com", _flat_metrics(formality=0.9)))
        for _ in range(min_samples):
            extractor._update_profile(_make_signal("casual@example.com", _flat_metrics(formality=0.1)))
...
class TestDraftContextUsesPerContactAverages:
    ...
    def _store_linguistic_profile(
        self,
        ums,
        global_formality: float,
    ...
    def test_uses_per_contact_label_when_available(self, db, user_model_store):
        ...
        assert "style with this contact" in result, "draft context should use per-contact label when data exists"
    ...
    def test_falls_back_to_global_when_no_per_contact_data(self, db, user_model_store):
        ...
        assert "general style" in result, "draft context should use 'general style' label when no per-contact data"
    ...
    def test_per_contact_formality_value_used(self, db, user_model_store):
        ...
        assert "formality=0.92" in result, "per-contact formality value should appear in draft context"
        assert "formality=0.30" not in result, "global formality should not replace per-contact value"
    ...
    def test_per_contact_below_min_samples_falls_back_to_global(self, db, user_model_store):
        ...
        assert "general style" in result, "should fall back to global style when per-contact samples < min"
