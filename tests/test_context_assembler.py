...
# ===========================================================================
# Test: _get_predictions_context()
# ===========================================================================


class TestPredictionsContext:
    """Tests for the _get_predictions_context() helper.

    Predictions are forward-looking intelligence items generated by the
    prediction engine. Only surfaced (was_surfaced=1), unresolved
    (resolved_at IS NULL), recent (created < 7 days), and non-filtered
    (filter_reason IS NULL) predictions are included in the briefing.
    """
        ...
        conn.execute(
            """INSERT INTO predictions
               (id, prediction_type, description, confidence, confidence_gate,
                was_surfaced, resolved_at, filter_reason, suggested_action)
               VALUES (?, ?, ?, ?, 'SUGGEST', ?, ?, ?, ?)""",
            (
                str(uuid.uuid4()),
                prediction_type,
    ...
    def test_predictions_context_empty_when_none(self, db, user_model_store):
        """Returns empty string when no predictions exist."""
        assembler = ContextAssembler(db, user_model_store)
        result = assembler._get_predictions_context()
        assert result == ""

    def test_predictions_context_includes_surfaced_unresolved(self, db, user_model_store):
        """Surfaced, unresolved predictions appear with type, description, and confidence."""
        with db.get_connection("user_model") as conn:
        ...
        assert "Follow up with client about proposal" in result
        assert "0.78" in result

    def test_predictions_context_excludes_non_surfaced(self, db, user_model_store):
        """Predictions that were not surfaced (was_surfaced=0) are excluded."""
        with db.get_connection("user_model") as conn:
            self._insert_prediction(
                conn,
                prediction_type="reminder",
                description="Hidden prediction",
                confidence=0.5,
                was_surfaced=False,
            )
    ...
    def test_predictions_context_excludes_resolved(self, db, user_model_store):
        """Resolved predictions (resolved_at IS NOT NULL) are excluded."""
        with db.get_connection("user_model") as conn:
            self._insert_prediction(
                conn,
                prediction_type="reminder",
                description="Already resolved prediction",
                confidence=0.8,
                was_surfaced=True,
        ...
        assembler = ContextAssembler(db, user_model_store)
        result = assembler._get_predictions_context()
        assert "Already resolved" not in result

    def test_predictions_context_excludes_filtered(self, db, user_model_store):
        """Predictions with a filter_reason are excluded from the context."""
        with db.get_connection("user_model") as conn:
            self._insert_prediction(
                conn,
                prediction_type="reminder",
                description="Filtered by reaction score",
                confidence=0.8,
                was_surfaced=True,
                filter_reason="low_reaction_score",
            )
    ...
    def test_predictions_context_includes_suggested_action(self, db, user_model_store):
        """Suggested actions are appended to the prediction line with a dash prefix."""
        with db.get_connection("user_model") as conn:
            self._insert_prediction(
                conn,
                prediction_type="reminder",
                description="Check in with Alice",
                confidence=0.75,
                was_surfaced=True,
                suggested_action="Send a quick message",
            )

        assembler = ContextAssembler(db, user_model_store)
        result = assembler._get_predictions_context()
        assert "Send a quick message" in result
        assert "suggested:" in result

    def test_predictions_sorted_by_confidence_desc(self, db, user_model_store):
        """Predictions are sorted by confidence descending (most reliable first)."""
        with db.get_connection("user_model") as conn:
            self._insert_prediction(conn, "reminder", "Low confidence", 0.40, True)
            self._insert_prediction(conn, "opportunity", "High confidence", 0.92, True)
            self._insert_prediction(conn, "risk", "Medium confidence", 0.65, True)

        assembler = ContextAssembler(db, user_model_store)
        result = assembler._get_predictions_context()

        high_pos = result.index("High confidence")
        med_pos = result.index("Medium confidence")
        low_pos = result.index("Low confidence")
        assert high_pos < med_pos < low_pos

    def test_predictions_in_briefing_context(self, db, user_model_store):
        """Predictions appear as a section in the full briefing context."""
        with db.get_connection("user_model") as conn:
            self._insert_prediction(
                conn,
                prediction_type="reminder",
        ...
        assembler = ContextAssembler(db, user_model_store)
        briefing = assembler.assemble_briefing_context()
        assert "Active predictions from the system:" in briefing
        assert "alice@example.com" in briefing

    def test_briefing_omits_predictions_section_when_empty(self, db, user_model_store):
        """When no qualifying predictions exist, the section is omitted entirely."""
        assembler = ContextAssembler(db, user_model_store)
        briefing = assembler.assemble_briefing_context()
        assert "Active predictions" not in briefing


# ===========================================================================
# Test: _get_insights_context()
# ===========================================================================


class TestInsightsContext:
    """Tests for the _get_insights_context() helper.

    Insights are cross-signal pattern discoveries from the InsightEngine.
    Only non-negative-feedback insights within their staleness TTL are included.
    """
    ...
    def test_insights_context_empty_when_none(self, db, user_model_store):
        """Returns empty string when no insights exist."""
        assembler = ContextAssembler(db, user_model_store)
        result = assembler._get_insights_context()
        assert result == ""

    def test_insights_context_includes_recent_high_confidence(self, db, user_model_store):
        """Recent, high-confidence insights appear with category and summary."""
        with db.get_connection("user_model") as conn:
            self._insert_insight(
                conn,
                insight_type="pattern",
                summary="You typically check email between 8-9 AM on weekdays",
                confidence=0.85,
    ...
    def test_insights_context_excludes_negative_feedback(self, db, user_model_store):
        """Insights with feedback='negative' (thumbs-down) are excluded."""
        with db.get_connection("user_model") as conn:
            self._insert_insight(
                conn,
                insight_type="pattern",
                summary="You love spreadsheets",
                confidence=0.90,
                category="behavioral_pattern",
                feedback="negative",
            )
    ...
    def test_insights_context_uses_category_as_label(self, db, user_model_store):
        """The category field is used as the display label when set."""
        with db.get_connection("user_model") as conn:
            self._insert_insight(
                conn,
                insight_type="generic_type",
    ...
    def test_insights_context_falls_back_to_type_when_no_category(self, db, user_model_store):
        """When category is empty, the type field is used as the display label."""
        with db.get_connection("user_model") as conn:
            self._insert_insight(
                conn,
                insight_type="communication_style",
                summary="You write casually",
                confidence=0.70,
                category="",
            )
    ...
    def test_insights_sorted_by_confidence_desc(self, db, user_model_store):
        """Insights are sorted by confidence descending (most reliable first)."""
        with db.get_connection("user_model") as conn:
            self._insert_insight(conn, "p", "Low confidence insight", 0.40, "low")
            self._insert_insight(conn, "p", "High confidence insight", 0.95, "high")
            self._insert_insight(conn, "p", "Medium confidence insight", 0.70, "med")

        assembler = ContextAssembler(db, user_model_store)
        result = assembler._get_insights_context()

        high_pos = result.index("High confidence insight")
        med_pos = result.index("Medium confidence insight")
        low_pos = result.index("Low confidence insight")
        assert high_pos < med_pos < low_pos

    def test_insights_in_briefing_context(self, db, user_model_store):
        """Insights appear as a section in the full briefing context."""
        with db.get_connection("user_model") as conn:
            self._insert_insight(
                conn,
                insight_type="pattern",
                summary="You send most emails on Tuesdays",
                confidence=0.80,
                category="behavioral_pattern",
            )
        ...
        assembler = ContextAssembler(db, user_model_store)
        briefing = assembler.assemble_briefing_context()
        assert "Behavioral patterns and insights:" in briefing
        assert "Tuesdays" in briefing

    def test_briefing_omits_insights_section_when_empty(self, db, user_model_store):
        """When no qualifying insights exist, the section is omitted entirely."""
        assembler = ContextAssembler(db, user_model_store)
        briefing = assembler.assemble_briefing_context()
        assert "Behavioral patterns and insights:" not in briefing


# ===========================================================================
# Test: _get_routines_context()
# ===========================================================================


class TestRoutinesContext:
    """Tests for the _get_routines_context() helper.

    Routines are habitual behavioral patterns from Layer 3 Procedural Memory.
    Only routines with consistency_score >= 0.5 are surfaced.
    """

    def test_routines_context_empty_when_none(self, db, user_model_store):
        """Returns empty string when no routines exist."""
        assembler = ContextAssembler(db, user_model_store)
    ...
    def test_routines_context_includes_high_consistency(self, db, user_model_store):
        ...
        assembler = ContextAssembler(db, user_model_store)
        result = assembler._get_routines_context()
        assert "Observed behavioral routines" in result
        assert "morning" in result
        assert "2 steps" in result
        assert "~30 min" in result
        assert "seen 25x" in result
        assert "consistency: 0.85" in result
    ...
    def test_routines_context_capped_at_5(self, db, user_model_store):
        ...
        assembler = ContextAssembler(db, user_model_store)
        result = assembler._get_routines_context()
        # Count the number of bullet lines
    ...
    def test_routines_in_briefing_context(self, db, user_model_store):
        ...
        assembler = ContextAssembler(db, user_model_store)
        briefing = assembler.assemble_briefing_context()
        assert "Observed behavioral routines (Layer 3 Procedural Memory):" in briefing
        assert "evening" in briefing

    def test_briefing_omits_routines_section_when_empty(self, db, user_model_store):
        """When no qualifying routines exist, the section is omitted entirely."""
        assembler = ContextAssembler(db, user_model_store)
        briefing = assembler.assemble_briefing_context()
        assert "Observed behavioral routines" not in briefing


# ===========================================================================
# Test: _get_semantic_facts_context() categorization
# ===========================================================================


class TestSemanticFactsContextCategorization:
    """Tests for the _get_semantic_facts_context() categorization and filtering logic.

    Facts are organized into three groups: values, behavioral patterns, and
    preferences. Relationship-specific keys and noise interest tokens are excluded.
    """

    def test_semantic_facts_groups_values_category(self, db, user_model_store):
        """Facts with category='values' appear under the 'Values:' heading."""
        user_model_store.update_semantic_fact(
        ...
        assert "weekday_only_work" in result

    def test_semantic_facts_groups_behavioral_patterns(self, db, user_model_store):
        """Facts with behavioral-pattern key prefixes appear under 'Behavioral patterns:'."""
        user_model_store.update_semantic_fact(
    ...
    def test_semantic_facts_groups_preferences(self, db, user_model_store):
        """Remaining non-values, non-behavioral facts appear under 'Preferences:'."""
        user_model_store.update_semantic_fact(
        ...
        assert "home_office" in result

    def test_semantic_facts_excludes_relationship_keys(self, db, user_model_store):
        """Per-contact relationship_balance_* and relationship_priority_* are excluded."""
        user_model_store.update_semantic_fact(
            key="relationship_balance_alice@work.com",
            category="relationship",
            value="balanced",
            confidence=0.9,
        )
        user_model_store.update_semantic_fact(
            key="relationship_priority_bob@work.com",
            category="relationship",
            value="high",
            confidence=0.9,
        )
        ...
        assembler = ContextAssembler(db, user_model_store)
        result = assembler._get_semantic_facts_context()
        assert "relationship_balance_" not in result
        assert "relationship_priority_" not in result
    ...
    def test_semantic_facts_excludes_noise_interest_tokens(self, db, user_model_store):
        ...
        assembler = ContextAssembler(db, user_model_store)
        result = assembler._get_semantic_facts_context()
        # None of these short noise tokens should appear
        assert "interest_here" not in result
        assert "interest_more" not in result
        assert "interest_free" not in result

    def test_semantic_facts_keeps_meaningful_interests(self, db, user_model_store):
        """interest_* facts with longer values (>6 chars) are kept."""
        user_model_store.update_semantic_fact(
    ...
    def test_semantic_facts_excludes_location_duplicates(self, db, user_model_store):
        """frequent_location_* and location_domain_* keys are excluded."""
        user_model_store.update_semantic_fact(
            key="frequent_location_home",
            category="location",
            value="123 Main St",
            confidence=0.9,
        )
        ...
        assembler = ContextAssembler(db, user_model_store)
        result = assembler._get_semantic_facts_context()
        assert "frequent_location_" not in result

    def test_semantic_facts_excludes_below_confidence_threshold(self, db, user_model_store):
        """Facts with confidence < 0.6 are excluded."""
        user_model_store.update_semantic_fact(
        ...
        assembler = ContextAssembler(db, user_model_store)
        result = assembler._get_semantic_facts_context()
        assert "low_confidence_fact" not in result

    def test_semantic_facts_empty_when_no_facts(self, db, user_model_store):
        """Returns empty string when no semantic facts exist."""
        assembler = ContextAssembler(db, user_model_store)
        result = assembler._get_semantic_facts_context()
        assert result == ""
...
# ===========================================================================
# Test: _get_mood_context() labels and thresholds
# ===========================================================================


class TestMoodContextLabels:
    """Tests for mood dimension labelling thresholds in _get_mood_context().

    The method converts 0-1 scalars to human-readable labels:
    - energy/stress: >=0.65 → high, 0.35-0.65 → moderate, <0.35 → low
    ...
    def test_mood_returns_empty_when_no_data(self, db, user_model_store):
        """Returns empty string when mood_history has no recent entries."""
        assembler = ContextAssembler(db, user_model_store)
        assert assembler._get_mood_context() == ""

    def test_mood_labels_high_energy(self, db, user_model_store):
        """Energy >= 0.65 is labelled 'high'."""
        with db.get_connection("user_model") as conn:
            self._insert_mood(conn, energy=0.80)
        assembler = ContextAssembler(db, user_model_store)
        result = assembler._get_mood_context()
        assert "energy_level=0.80 (high)" in result

    def test_mood_labels_moderate_energy(self, db, user_model_store):
        """Energy between 0.35 and 0.65 is labelled 'moderate'."""
        with db.get_connection("user_model") as conn:
            self._insert_mood(conn, energy=0.50)
        assembler = ContextAssembler(db, user_model_store)
        result = assembler._get_mood_context()
        assert "energy_level=0.50 (moderate)" in result

    def test_mood_labels_low_energy(self, db, user_model_store):
        """Energy < 0.35 is labelled 'low'."""
        with db.get_connection("user_model") as conn:
            self._insert_mood(conn, energy=0.20)
        assembler = ContextAssembler(db, user_model_store)
        result = assembler._get_mood_context()
        assert "energy_level=0.20 (low)" in result

    def test_mood_labels_positive_valence(self, db, user_model_store):
        """Emotional valence >= 0.65 is labelled 'positive'."""
        with db.get_connection("user_model") as conn:
            self._insert_mood(conn, valence=0.80)
        assembler = ContextAssembler(db, user_model_store)
        result = assembler._get_mood_context()
        assert "emotional_valence=0.80 (positive)" in result

    def test_mood_labels_neutral_valence(self, db, user_model_store):
        """Emotional valence between 0.35 and 0.65 is labelled 'neutral'."""
        with db.get_connection("user_model") as conn:
            self._insert_mood(conn, valence=0.50)
        assembler = ContextAssembler(db, user_model_store)
        result = assembler._get_mood_context()
        assert "emotional_valence=0.50 (neutral)" in result

    def test_mood_labels_negative_valence(self, db, user_model_store):
        """Emotional valence < 0.35 is labelled 'negative'."""
        with db.get_connection("user_model") as conn:
            self._insert_mood(conn, valence=0.20)
        assembler = ContextAssembler(db, user_model_store)
        result = assembler._get_mood_context()
        assert "emotional_valence=0.20 (negative)" in result

    def test_mood_omits_low_confidence(self, db, user_model_store):
        """Low-confidence mood readings (< 0.3) omit the confidence field."""
        with db.get_connection("user_model") as conn:
            self._insert_mood(conn, confidence=0.1)
        assembler = ContextAssembler(db, user_model_store)
        result = assembler._get_mood_context()
        assert "confidence=" not in result

    def test_mood_includes_high_confidence(self, db, user_model_store):
        """High-confidence mood readings (>= 0.3) include the confidence field."""
        with db.get_connection("user_model") as conn:
            self._insert_mood(conn, confidence=0.85)
        assembler = ContextAssembler(db, user_model_store)
        result = assembler._get_mood_context()
        assert "confidence=0.85" in result

    def test_mood_stale_data_returns_empty(self, db, user_model_store):
        """Mood snapshots older than 24 hours are excluded."""
        ...
        with db.get_connection("user_model") as conn:
            conn.execute(
                """INSERT INTO mood_history
                   (timestamp, energy_level, stress_level, emotional_valence,
                    social_battery, cognitive_load, confidence, trend)
                   VALUES (?, 0.8, 0.3, 0.7, 0.5, 0.3, 0.8, 'stable')""",
                (old_ts,),
            )
        assembler = ContextAssembler(db, user_model_store)
        assert assembler._get_mood_context() == ""
...
# ===========================================================================
# Test: Draft context — conversation history and per-contact style
# ===========================================================================


class TestDraftContextConversationHistory:
    ...
    def _insert_episode(conn, content_summary, contacts, topics=None,
        ...
        conn.execute(
            """INSERT INTO episodes
               (id, timestamp, event_id, interaction_type, content_summary,
                contacts_involved, topics, inferred_mood)
               VALUES (?, ?, ?, ?, ?, ?, ?, '{}')""",
            (
                str(uuid.uuid4()),
                timestamp,
    ...
    def test_draft_includes_conversation_history(self, db, user_model_store):
        """Recent episodes with the contact appear as conversation history."""
        with db.get_connection("user_model") as conn:
            self._insert_episode(
                conn,
                "Discussed Q1 budget with finance team",
                contacts=["alice@work.com"],
                topics=["budget", "Q1"],
            )

        assembler = ContextAssembler(db, user_model_store)
        result = assembler.assemble_draft_context(
            contact_id="alice@work.com",
            channel="email",
            incoming_message="Any updates on the budget?",
        )
        assert "Recent conversation history with this contact:" in result
        assert "Q1 budget" in result

    def test_draft_history_includes_topics(self, db, user_model_store):
        """Episode topics are shown in the conversation history."""
        with db.get_connection("user_model") as conn:
            self._insert_episode(
                conn,
                "Reviewed project proposal",
                contacts=["bob@work.com"],
                topics=["project", "proposal", "review"],
            )
        ...
        result = assembler.assemble_draft_context(
            contact_id="bob@work.com",
            channel="email",
            incoming_message="Can we discuss the proposal?",
        )
        assert "topics:" in result
    ...
    def test_draft_per_contact_style_preferred_over_global(self, db, user_model_store):
        ...
        assembler = ContextAssembler(db, user_model_store)
        result = assembler.assemble_draft_context(
            contact_id="boss@work.com",
            channel="email",
            incoming_message="Status update?",
        )
        assert "formality=0.85" in result
        assert "with this contact" in result
    ...
    def test_draft_falls_back_to_global_style(self, db, user_model_store):
        ...
        assembler = ContextAssembler(db, user_model_store)
        result = assembler.assemble_draft_context(
            contact_id="new_contact@work.com",
            channel="email",
            incoming_message="Hello",
        )
        assert "formality=0.50" in result
    ...
    def test_draft_formality_delta_note(self, db, user_model_store):
        ...
        assembler = ContextAssembler(db, user_model_store)
        result = assembler.assemble_draft_context(
            contact_id="formal_contact@work.com",
            channel="email",
            incoming_message="Hi",
        )
    ...
    def test_draft_includes_inbound_contact_style(self, db, user_model_store):
        ...
        assembler = ContextAssembler(db, user_model_store)
        result = assembler.assemble_draft_context(
            contact_id="casual@friend.com",
            channel="chat",
            incoming_message="yo",
        )
        assert "Contact's writing style" in result
        assert "formality=0.20" in result
...
class TestGracefulDegradation:
    """Tests for fail-open error handling across all context assembly methods.

    The ContextAssembler is designed to never crash the briefing or draft or
    search context assembly, even when individual data sources fail.
    """

    def test_briefing_survives_corrupted_user_model(self, db, user_model_store):
        """Briefing context still works when UserModelStore methods raise."""
        from unittest.mock import MagicMock

        from storage.user_model_store import UserModelStore as _UMS

        mock_ums = MagicMock(spec=_UMS)
        mock_ums.get_routines.side_effect = Exception("DB corrupted")
        mock_ums.get_semantic_facts.side_effect = Exception("DB corrupted")
        mock_ums.get_signal_profile.return_value = None
        mock_ums.get_communication_template.return_value = None

        assembler = ContextAssembler(db, mock_ums)
        result = assembler.assemble_briefing_context()

        # Core structural elements should still be present
        assert isinstance(result, str)
        assert "Current time:" in result
        assert "User preferences:" in result

    def test_search_context_survives_facts_error(self, db, user_model_store):
        """Search context still returns a valid string when facts fail to load."""
        from unittest.mock import MagicMock

        from storage.user_model_store import UserModelStore as _UMS

        mock_ums = MagicMock(spec=_UMS)
        mock_ums.get_semantic_facts.side_effect = Exception("Facts unavailable")
        mock_ums.get_signal_profile.return_value = None

        assembler = ContextAssembler(db, mock_ums)
        result = assembler.assemble_search_context("test query")

        assert "test query" in result
        assert "Current time:" in result

    def test_draft_context_survives_inbound_profile_error(self, db, user_model_store):
        """Draft context still works when the inbound linguistic profile fails.

        The inbound profile lookup (Layer 4) is wrapped in try/except in the
        production code, so an error there degrades gracefully.  Layers 2 and 3
        (relationship and outbound linguistic) are NOT wrapped, so we test the
        actual fail-open path: inbound profile error.
        """
        from unittest.mock import MagicMock
        ...
        def _side_effect(profile_type):
            if profile_type == "linguistic_inbound":
                raise Exception("Inbound profile corrupted")
            return None

        mock_ums.get_signal_profile.side_effect = _side_effect
