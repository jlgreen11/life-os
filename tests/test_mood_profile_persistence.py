...
class TestMoodProfilePersistenceEmailReceived:
    ...
    def test_email_received_with_body_produces_signals(self, engine):
        ...
        """
        event = _make_email_received("Hello, I wanted to follow up on our meeting from last week.", hour=10)

        signals = engine.extract(event)

        assert len(signals) == 1, (
            "email.received with body text must produce signals; got empty list — circadian_energy proxy not firing"
        )
        assert signals[0]["type"] == "mood_signal"
        mood_signals = signals[0]["signals"]
        assert len(mood_signals) >= 1, (
            "At least circadian_energy signal must be extracted from a timestamped email.received event with body text"
        )
    ...
    def test_email_received_mood_signals_profile_exists_after_extract(self, engine, user_model_store):
        ...
        assert profile is not None, (
            "mood_signals profile must exist in the database after processing an email.received event with body text"
        )
    ...
    def test_email_received_profile_has_recent_signals_key(self, engine, user_model_store):
        ...
        profile = user_model_store.get_signal_profile("mood_signals")
        assert "recent_signals" in profile["data"], "mood_signals profile data must contain 'recent_signals' key"
    ...
    def test_email_received_circadian_energy_in_persisted_profile(self, engine, user_model_store):
        ...
        """
        event = _make_email_received("Good morning, can we schedule a call this week?", hour=9)
    ...
    def test_negative_email_produces_incoming_negative_language_signal(self, engine, user_model_store):
        ...
        assert "incoming_negative_language" in signal_types, (
            "Email with negative words must produce an incoming_negative_language signal in the persisted profile"
        )
...
class TestMoodProfilePersistenceMultipleEvents:
    ...
    def test_multiple_emails_accumulate_signals_in_profile(self, engine, user_model_store):
        ...
        assert len(profile["data"]["recent_signals"]) >= 3, (
            "Three email.received events must accumulate at least 3 signals (one circadian_energy per event minimum)"
        )
    ...
    def test_profile_samples_count_increments_per_write(self, engine, user_model_store):
        ...
        assert profile["samples_count"] >= 2, "Each successful write to mood_signals must increment samples_count"
...
class TestPostWriteVerification:
    ...
    def test_post_write_profile_readable_after_extract(self, engine, user_model_store):
        ...
        """
        event = _make_email_received("Please review the attached document and share your thoughts.", hour=14)
    ...
    def test_no_logger_error_for_normal_email_event(self, engine, caplog):
        ...
        """
        event = _make_email_received("Quick question about the project timeline — can we chat tomorrow?", hour=10)
        ...
        assert len(persistence_errors) == 0, (
            f"No post-write verification error expected for a normal email.received event; got: {persistence_errors}"
        )
    ...
    def test_signal_data_json_serializable(self, engine):
        ...
        events = [
            ...
            _make_email_received("I'm frustrated and stressed about this urgent problem.", hour=14),
            {
                "type": EventType.SLEEP_RECORDED.value,
