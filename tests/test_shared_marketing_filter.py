...
class TestNoReplyPatterns:
    ...
    def test_noreply_blocked(self, addr):
        ...
        assert is_marketing_or_noreply(addr) is True, f"Expected {addr!r} to be filtered as automated, but it wasn't"
...
class TestThirdPartyEmailPlatforms:
    ...
    def test_third_party_platform_blocked(self, addr):
        ...
        assert is_marketing_or_noreply(addr) is True, f"Expected {addr!r} to be filtered (ESP domain), but it wasn't"
...
class TestHumanAddressesNotBlocked:
    ...
    def test_human_address_not_blocked(self, addr):
        ...
        assert is_marketing_or_noreply(addr) is False, f"False positive: {addr!r} was incorrectly filtered as automated"
...
class TestConsistencyAcrossCallSites:
    ...
    def test_previously_diverged_patterns_now_consistent(self, addr):
        ...
        assert shared_result is True, f"Shared filter should block {addr!r}"
        assert extractor_result == shared_result, f"RelationshipExtractor diverges from shared filter for {addr!r}"
        assert tracker_result == shared_result, f"BehavioralAccuracyTracker diverges from shared filter for {addr!r}"
