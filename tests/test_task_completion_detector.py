...
def create_task(db, task_id, title, description="", created_at=None, status="pending"):
    ...
    with db.get_connection("state") as conn:
        ...
        """,
            (task_id, title, description, status, created_at.isoformat(), created_at.isoformat()),
        )
...
class TestInactivityBasedCompletion:
    ...
    @pytest.mark.asyncio
    async def test_marks_inactive_task_complete_even_with_unrelated_system_activity(self, detector, db, base_time):
    ...
    async def test_respects_inactivity_threshold(self, detector, db, base_time):
        ...
        create_task(db, task_id_at_threshold, "At threshold", created_at=base_time - timedelta(days=7))

        # Task created just past threshold (7 days + 1 hour) — should complete.
        task_id_past_threshold = str(uuid.uuid4())
        create_task(db, task_id_past_threshold, "Past threshold", created_at=base_time - timedelta(days=7, hours=1))
...
class TestUpdateEpisodeOutcome:
    ...
    def test_updates_episode_when_found(self, db, event_bus, user_model_store):
        ...
        task_manager = MagicMock()
        task_manager.complete_task = AsyncMock()
        det = TaskCompletionDetector(db, task_manager, event_bus, user_model_store=user_model_store)
        ...
        with db.get_connection("user_model") as conn:
            conn.execute(
                ...
                "VALUES (?, ?, ?, ?, ?)",
                (episode_id, event_id, "email", datetime.now(timezone.utc).isoformat(), "Test episode"),
            )
...
class TestDirectStrategyMethods:
    ...
    async def test_activity_detection_returns_count(self, detector, db, base_time):
        ...
        task_id = str(uuid.uuid4())
        create_task(db, task_id, "Send quarterly report manager", created_at=base_time - timedelta(hours=3))
        create_event(
            db,
            "email.sent",
            {
    ...
    async def test_inactivity_detection_skips_recent(self, detector, db, base_time):
        ...
        create_task(db, str(uuid.uuid4()), "Recent task", created_at=base_time - timedelta(days=3))
    ...
    async def test_stale_detection_directly(self, detector, db, base_time):
        ...
        task_id = str(uuid.uuid4())
        create_task(db, task_id, "Ancient task", created_at=base_time - timedelta(days=35))
    ...
    async def test_stale_detection_skips_20_day_task(self, detector, db, base_time):
        ...
        create_task(db, str(uuid.uuid4()), "Not yet stale", created_at=base_time - timedelta(days=20))
