...
def test_check_and_rebuild_passes_50k_event_limit(db, user_model_store):
    ...
    assert kwargs["event_limit"] == 50000, f"Expected event_limit=50000, got {kwargs['event_limit']}"
...
def test_rebuild_processes_all_fetched_events(db, user_model_store):
    ...
    assert result["events_processed"] == 100, f"Expected all 100 events processed, got {result['events_processed']}"
...
def test_type_filter_unions_multiple_missing_profiles(db, user_model_store):
    ...
    result = pipeline.rebuild_profiles_from_events(
        event_limit=1000,
        missing_profiles=["spatial", "linguistic_inbound"],
    )

    # Both ios.context.update and email.received should be fetched.
    # system.rule.triggered should still be excluded.
    assert result["events_processed"] == 2, f"Expected 2 events, got {result['events_processed']}"
...
def test_profile_event_types_covers_all_expected_profiles():
    ...
    for profile_name in expected:
        assert profile_name in PROFILE_EVENT_TYPES, f"PROFILE_EVENT_TYPES missing entry for '{profile_name}'"
        assert len(PROFILE_EVENT_TYPES[profile_name]) > 0, f"PROFILE_EVENT_TYPES['{profile_name}'] is empty"
