...
async def test_search_life_uses_vector_store(db, user_model_store, event_store):
    ...
    mock_vector_store.search.return_value = [
        {"doc_id": "evt-semantic-1", "text": "Mike mentioned the Denver project is ahead of schedule.", "score": 0.8542}
    ]
    ...
    with patch.object(engine.context, "assemble_search_context") as mock_context:
        ...
        with patch.object(engine, "_query_local", new_callable=AsyncMock) as mock_local:
            ...
            mock_vector_store.search.assert_called_once_with("What did Mike say about Denver?", limit=20)
    ...
    event_store.store_event(
        {
            ...
            "priority": "normal",
            "payload": {"snippet": "Mom also sent a brownie recipe last week."},
            "metadata": {},
        }
    )
...
async def test_search_life_handles_snippet_fallback(db, user_model_store, event_store):
    ...
    mock_vector_store = Mock(spec=VectorStore)
    mock_vector_store.search.return_value = [{"doc_id": "evt-no-snippet", "text": "Team Meeting", "score": 0.85}]
...
async def test_search_life_truncates_snippets_to_100_chars(db, user_model_store, event_store):
    ...
    mock_vector_store = Mock(spec=VectorStore)
    mock_vector_store.search.return_value = [{"doc_id": "evt-long", "text": long_snippet, "score": 0.90}]
    ...
    event_store.store_event(
        {
            ...
            "timestamp": "2026-02-10T10:00:00Z",
            "priority": "normal",
            "payload": {"subject": "Quarterly Report", "snippet": "The Q1 quarterly report shows 25% growth."},
            "metadata": {},
        }
    )
...
async def test_search_life_handles_missing_event_in_db(db, user_model_store):
    ...
    mock_vector_store = Mock(spec=VectorStore)
    mock_vector_store.search.return_value = [{"doc_id": "evt-deleted", "text": "deleted content", "score": 0.95}]
        ...
        event_id = event_store.store_event(
            {
                ...
                "timestamp": "2026-02-10T10:00:00Z",
                "priority": "normal",
                "payload": {"snippet": "Machine learning models require large datasets for training."},
                "metadata": {},
            }
        )
