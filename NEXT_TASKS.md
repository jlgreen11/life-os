...
<!-- NOTE (2026-04-22 iter29 / iter30): Progress on option (b) — `pytest-cov>=5.0.0` now listed in `requirements.txt` (alongside existing pytest / pytest-asyncio). When the human creates `.venv` and runs `pip install -r requirements.txt`, pytest-cov will be available and this task can close. Iter30 re-verified: `.venv` still absent, `python3.12 -c "import pytest_cov"` → ModuleNotFoundError, `which ruff` → not found. Task still `- [ ]` until env exists. -->
...
<!-- NOTE (2026-04-22 iter29 / iter30): Progress on the mypy half — `mypy>=1.8.0` now listed in `requirements.txt` (alongside existing ruff + pre-commit). When the human creates `.venv` and runs `pip install -r requirements.txt`, both ruff and mypy will be available and this task can close. Ruff half also requires the venv to exist so auto-fixes can be verified against a working test suite. Iter30 re-verified: `which ruff` → not found; `python3.12 -m ruff` → ModuleNotFoundError. Task still `- [ ]` until env exists. -->
