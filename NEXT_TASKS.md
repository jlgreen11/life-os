...
<!-- NOTE (2026-04-22 / last verified 2026-04-23 iter59): Progress on the mypy half — `mypy>=1.8.0` now listed in `requirements.txt` (alongside existing ruff + pre-commit). When the human creates `.venv` and runs `pip install -r requirements.txt`, both ruff and mypy will be available and this task can close. Ruff half also requires the venv to exist so auto-fixes can be verified against a working test suite. Re-verified through iter29 / iter30 / iter32 / iter45 / iter47 / iter48 / iter49 / iter51 / iter52 / iter55 / iter56 / iter58 / iter59: `which ruff` → not found; `python3.12 -c "import mypy"` → ModuleNotFoundError. Env unchanged since iter30. -->
