...
<!-- NOTE (2026-04-22 iter29 / iter30 / iter32 / iter45 / iter47): Progress on option (b) — `pytest-cov>=5.0.0` now listed in `requirements.txt` (alongside existing pytest / pytest-asyncio). When the human creates `.venv` and runs `pip install -r requirements.txt`, pytest-cov will be available and this task can close. Iter47 re-verified (2026-04-22): `.venv` still absent, `python3.12 -c "import pytest_cov"` → ModuleNotFoundError, `python3.12 -c "import fastapi"` → ModuleNotFoundError, `which ruff` → not found, `python3.12 -c "import mypy"` → ModuleNotFoundError. Task still `- [ ]` until env exists; no change since iter30 — agent is exiting PARTIAL rather than burning further iterations on re-verify. -->
